...
int index_save(const Index *index) {
    // Sort a heap-allocated copy (struct is too large for stack)
    ...
    *sorted = *index;
    ...
    char tmp[256];
    snprintf(tmp, sizeof(tmp), ".pes/index.tmp");
    FILE *f = fopen(tmp, "w");
...
int index_add(Index *index, const char *path) {
    ...
    if (fread(buf, 1, st.st_size, f) != (size_t)st.st_size && st.st_size > 0) {
